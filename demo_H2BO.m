...
Y2 = Y2./scfact;
imgVec_bands = reshape(Y2, [numRows*numCols numSpectra]);

tic

%% Hierarchical Homogeneity-Based Oversegmentation (H2BO)
fprintf('-- Hierarchical Homogeneity-Based Oversegmentation (H2BO)\n');

sppx = H2BO(Y2, slic_size, slic_reg, tau_outliers, tau_homog);

show_segments(Y2, sppx, [37 16 10]);
save(strcat(path_output,'sppx.mat'),'sppx');

%% Homogeneity test - Final
numSuperpixels = length(unique(sppx(:)));
fprintf('num_sppx = %d\n\n',numSuperpixels);
sppx_labels = unique(sppx);
[labels_homog, labels_heter, sppx_median, sppx_dist, sppx_dist_avg, sppx_dist_avg_trim, sppx_dist_max, sppx_dist_max_trim, sppx_dist_ratio] = homog_median_dist(Y2, sppx, sppx_labels, tau_homog, tau_outliers,'T','F');
img_homog_percent = 100*length(labels_homog)/length(sppx_labels);
img_homog_mean = mean(sppx_dist_ratio);
fprintf('img_homog_percent = %.1f %% \n', img_homog_percent);
fprintf('img_homog_mean = %.3f \n', img_homog_mean);
